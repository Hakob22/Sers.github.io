...
function updateDepth(book, newPage) {
	...
	if (newPage>3)
		$('.sg-book .p2 .depth').css({
			width: depthWidth,
			left: 20 - depthWidth
		});
	else
		$('.sg-book .p2 .depth').css({width: 0});

		depthWidth = 16*Math.min(1, (pages-page)*2/pages);

	if (newPage<pages-3)
		$('.sg-book .p19 .depth').css({
			width: depthWidth,
			right: 20 - depthWidth
		});
	else
		$('.sg-book .p19 .depth').css({width: 0});
...
function loadPage(page) {
	...
	$.ajax({url: 'pages/page' + page + '.html'}).
			...
			$('.sg-book .p' + page).html(pageHtml.replace('samples/sers-gyux/',));
		});
...
function zoomThis(pic) {
	...
	var	position, translate,
		...
		$('.sg-book').data().zoomIn = true;

		$('.sg-book').turn('disable', true);
...
function zoomOut() {
	...
	var transitionEnd = $.cssTransitionEnd(),
		completeTransition = function(e) {
			$('#book-zoom').unbind(transitionEnd);
			$('.sg-book').turn('disable', false);
			$('body').css({'overflow': 'auto'});
			moveBar(false);
		};

	$('.sg-book').data().zoomIn = false;
